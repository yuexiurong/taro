...
//缓存数据 H5 小程序
//缓存的这个数据可以是数组/对象  当前我们是缓存的对象{food.id:{菜品信息 Num},}
import Taro from '@tarojs/taro';
import Event from './events'
const foodKey="taro_meituan"//定义缓存的key值
...
//设置菜品数量 当加菜或者减菜时触发 Num是由加减菜组件自身的state存储的
//当件得数量等于0时 要删除菜品的数据结构
...
export function getAllFoodInfo(){
  ...
  if(store){
    ...
    Object.keys(store).map((item)=>{
      if(store[item]){
        if(store[item].price&&store[item].Num){
          allPrice=allPrice+(store[item].price*store[item].Num);
          allNum=allNum+store[item].Num;
        }
      }
    })
  }
